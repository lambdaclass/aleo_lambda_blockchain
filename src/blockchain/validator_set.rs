...
impl ValidatorSet {
    ...
    pub fn rewards(&self) -> Vec<(jaleo::Field, jaleo::JAleoRecord)> {
...
mod tests {
    ...
    fn rewards_are_deterministic() {
        ...
        records11.sort_by_key(|k| k.0);
        records21.sort_by_key(|k| k.0);
        ...
        records12.sort_by_key(|k| k.0);
        records22.sort_by_key(|k| k.0);
    ...
    fn decrypt_rewards(
        ...
        rewards: &[(jaleo::Field, jaleo::JAleoRecord)],
    ) -> u64 {
        rewards
            .iter()
            .filter(|(_, record)| {
